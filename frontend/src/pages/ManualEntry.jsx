...
export default function ManualEntry() {
  ...
  useEffect(() => {
    const t = setTimeout(async () => {
      ...
      try {
        ...
        const r = await fetch(`${API_BASE}/courses/search?q=${encodeURIComponent(q.trim())}`);
  ...
  async function loadCourse(c) {
    ...
    setSuggestions([]);
    const r = await fetch(`${API_BASE}/courses/${encodeURIComponent(c.subject)}/${encodeURIComponent(c.number)}`);
    const data = await r.json();
    setCourse(data);
    setSectionKey("");
  ...
  const lectures = useMemo(() => {
    if (!section) return [];
    return (section.offerings || []).filter(o => o.type.toUpperCase().startsWith("LECT"))
  ...
  const labOrTutr = useMemo(() => {
    ...
    return (section.offerings || [])
      .filter(o => o.type.toUpperCase().startsWith("LAB") || o.type.toUpperCase().startsWith("TUTR"))
      .map(o => ({ id: `${o.type.toUpperCase()}-${o.number || "NA"}`, ...o }));
  }, [section]);
  ...
  function buildPending() {
    ...
    if (chosenLab) {
      for (const m of chosenLab.meetings || []) {
        chosen.push({
          ...base,
          type: chosenLab.type.toUpperCase().startsWith("TUTR") ? "TUTR" : "LAB",
          days: m.day || "",
          startTime: m.startTime || "",
          duration: m.duration || null,
          room: m.room || "",
  ...
  async function saveAll() {
    if (!pendingItems.length) { setMsg("Nothing to save"); return; }
    try {
    ...
    <div>
      ...
      <div style={s.searchWrap}>
        <input
          ...
          placeholder="Search by code or name (e.g., EECS 3482, Security)"
        />
      ...
      {suggestions.length > 0 && (
        <div style={s.suggestBox}>
          {suggestions.map(c => (
            <div key={`${c.subject}-${c.number}`} style={s.suggestItem} onClick={() => loadCourse(c)}>
      ...
      {course && (
        <div style={{ marginTop: 16 }}>
          ...
          <div style={s.row}>
            ...
            <select style={s.select} value={sectionKey} onChange={e => { setSectionKey(e.target.value); setSelectedLectureId(""); setSelectedLabId(""); }}>
              <option value="">-- Choose term & section --</option>
              {sections.map(sct => (
                <option key={`${sct.term}-${sct.letter}`} value={`${sct.term}-${sct.letter}`}>
                  {sct.term} — Section {sct.letter} {sct.instructor ? `(${sct.instructor})` : ""}
                </option>
              ))}
            </select>
          </div>
          ...
          {!!lectures.length && (
            <div style={s.row}>
              ...
              <select style={s.select} value={selectedLectureId} onChange={e => setSelectedLectureId(e.target.value)}>
                <option value="">-- Choose lecture --</option>
                {lectures.map(l => (
                  <option key={l.id} value={l.id}>
                    {l.type} {l.number || ""} — {l.meetings?.map(m => `${m.day} ${m.startTime}`).join(", ")}
                  </option>
                ))}
              </select>
            </div>
          ...
          {!!labOrTutr.length && (
            <div style={s.row}>
              ...
              <select style={s.select} value={selectedLabId} onChange={e => setSelectedLabId(e.target.value)}>
          ...
          {pendingItems.length > 0 && (
            <>
              ...
              <table border="1" cellPadding="6">
                <thead>
                  <tr>
                    ...
                    <th>Day</th><th>Start</th><th>Duration</th><th>Room</th>
                  </tr>
                </thead>
                <tbody>
              ...
              <div style={{ display: "flex", gap: 8, marginTop: 10 }}>
                <button onClick={saveAll}>Save to My Schedule</button>
                <button onClick={() => nav("/my-schedule")}>See my schedule</button>
              </div>
            </>
          )}

          {msg && <p style={{ color: msg.includes("Saved") ? "green" : "red" }}>{msg}</p>}
        </div>
      )}
    </div>
  );
...
const s = {
  ...
  suggestBox: {
    width: 520, background: "#fff", border: "1px solid #ddd", marginTop: 6, borderRadius: 4,
    maxHeight: 220, overflowY: "auto"
  },
  suggestItem: { padding: 8, borderBottom: "1px solid #eee", cursor: "pointer" },
